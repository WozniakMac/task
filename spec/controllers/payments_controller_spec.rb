...
RSpec.describe PaymentsController do
  ...
  describe 'PUT #update' do
    let!(:payment) { create :payment, payment_date: '2016-10-10', paid: false, student: student }
    let(:payment_date) { Date.parse '2016-11-11' }

    let!(:params) do
      { id: payment.id, payment: { payment_date: payment_date } }
    end
    ...
    context 'success' do
      ...
      context 'updates payment' do
        subject { -> { put :update, params } }
        it { is_expected.to change{ payment.reload.payment_date }.to(payment_date) }
      end
    end
